...
```{r sampleSummary, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if (!is.null(args$sampleSummary)) {
  df <- readRDS(args$sampleSummary)
  kable(df, "latex",  linesep = "", booktabs = T, caption = "sample meta data")
} else {
  cat("No Summary Given")
}
...
dash <- findRandomVal(frag, inExon, inTU)
...
ggplot(gene.dist, aes(y = sample)) +
  ...
  scale_x_continuous(labels = scales::label_percent(accuracy = .1, scale = 1), 
                     limits = c(0, 100)) +
  geom_vline(xintercept = dash[[1]], linetype = "dashed") +
  geom_vline(xintercept = dash[[2]], linetype = "dashed") +
